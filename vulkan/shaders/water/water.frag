...
void main() 
{
	...
	vec3 halfVector = normalize(-viewSpacePosition) + normalize(lightDirection);
	col += pow(max(dot(normalize(halfVector), normal), 0.0f), 128.0f) * lightColor;
	col *= vec3(0.5, 0.7, 1.0f);
