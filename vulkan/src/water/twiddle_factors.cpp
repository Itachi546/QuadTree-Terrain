...
TwiddleFactors::TwiddleFactors(Context* context, uint32_t N)
{
	// Setup pipeline
	{
		std::string code = load_file("spirv/twiddleFactors.comp.spv");
	...
	// vertical dimension of N (log2(N) * N)
	{
		...
		m_twiddleTexture = Device::create_texture(desc);
	}
	...
	bindings->set_storage_image(m_twiddleTexture, 0);
...
void TwiddleFactors::create_twiddle_texture(Context* context)
{
	create_twiddle_texture(context, pipeline, bindings, 256);
}

void TwiddleFactors::destroy()
{
	Device::destroy_buffer(indicesBuffer);
	Device::destroy_shader_bindings(bindings);
	Device::destroy_pipeline(pipeline);
	Device::destroy_texture(m_twiddleTexture);
}

void TwiddleFactors::create_twiddle_texture(Context* context, Pipeline* pipeline, ShaderBindings* bindings, uint32_t N)
{
